...
public class CustomersRepository {
    ...
    public static ArrayList<Customers> selectByUsernameAndPassword(String username, String password) {
        ArrayList<Customers> lst = new ArrayList<Customers>();
        String SQL_QUERY = "from Customers item where item.username='" + username + "' and item.password='" + password + "'";
        Query query = (Query) HibernateUtil.getSessionFactory().openSession().createQuery(SQL_QUERY);
        for (Iterator it = query.iterate(); it.hasNext();) {
            lst.add((Customers) it.next());
        }
